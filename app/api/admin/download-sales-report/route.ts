  ...
  try {
    ...
    if (adminUserCookie) {
      try {
        ...
        console.log("[v0] Could not parse admin cookie")
      }
    }
    ...
    console.log("[v0] Download PDF - Received params:", { format, period, selectedMonth, selectedYear, adminName })
    ...
    // Footer with generation date and admin info
    const pageCount = doc.getNumberOfPages()
    for (let i = 1; i <= pageCount; i++) {
      doc.setPage(i)
      doc.setFontSize(8)
      doc.text(`Generated by ${adminName} on ${new Date().toLocaleString()} - Page ${i} of ${pageCount}`, 105, 290, { align: "center" })
    }
