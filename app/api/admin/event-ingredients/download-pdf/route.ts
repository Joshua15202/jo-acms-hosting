  ...
  try {
    ...
    if (appointmentId) {
      ...
      for (let i = 1; i <= pageCount; i++) {
        ...
        doc.setFontSize(8)
        doc.setTextColor(150, 150, 150)
        doc.text(`Jo-ACMS - Kitchen Ingredient List | Generated by ${adminName} on ${new Date().toLocaleString()}`, 14, 290)
    ...
    for (let i = 1; i <= pageCount; i++) {
      ...
      doc.setFontSize(8)
      doc.setTextColor(150, 150, 150)
      doc.text(`Jo-ACMS - Event Ingredients Report | Generated by ${adminName} on ${new Date().toLocaleString()}`, 14, 290)
      doc.text(`Page ${i} of ${pageCount}`, 195, 290, { align: "right" })
    }
