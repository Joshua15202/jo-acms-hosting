...
export class MySQLChatService {
  static async createChatSession(userId: string, title = "New Chat"): Promise<ChatSession | null> {
    try {
      const id = `chat_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`
      const now = new Date().toISOString()

      await query("INSERT INTO chat_sessions (id, user_id, title, created_at, updated_at) VALUES (?, ?, ?, ?, ?)", [
        id,
        userId,
        title,
        now,
  ...
  static async getChatSessions(userId: string): Promise<ChatSession[]> {
    try {
      const rows = await query("SELECT * FROM chat_sessions WHERE user_id = ? ORDER BY updated_at DESC", [userId])
  ...
  static async getMessages(sessionId: string): Promise<Message[]> {
    try {
      const rows = await query("SELECT * FROM messages WHERE session_id = ? ORDER BY created_at ASC", [sessionId])
  ...
  static async saveMessage(
    ...
    try {
      ...
      await query("UPDATE chat_sessions SET updated_at = ? WHERE id = ?", [now, sessionId])
